    ...
    try:
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                if "fetch_mind_kernel_core" not in tool_names:
                    print("Error: fetch_mind_kernel_core not found!", file=sys.stderr)
                    sys.exit(1)
                ...
                # Call tool
                print(f"Calling fetch_mind_kernel_core with userId='{user_id}'...", file=sys.stderr)
                result = await session.call_tool("fetch_mind_kernel_core", arguments={"userId": user_id})
